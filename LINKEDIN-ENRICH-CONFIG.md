...
| Allowed PATCH fields | See `schemas/crm_leads_enrich_mapping.sql` — enforced inside `tools/crm-leads-update.js`'s `ALLOWED_TOP_KEYS`, not agent reasoning. Each allowed value is then shape-checked against `schemas/CRM_Leads_Field_Reference.json` (the CRM's column `field_type` + validation regex + enumerated options): a value that doesn't fit its column is dropped and reported as `invalid_keys`, never sent. `employees` must be a whole number (lower bound of a size range); `firstname`/`lastname` must match `^[a-zA-Z ]*$`; `secondary_email` must be a valid email; `tags` a string array. Tool fails closed if the reference file is missing |
