...
| Navigation hops | `profile,company` — after the source URL, also load the poster's (or, for a resolved reshare, the original poster's) profile page (plus its Contact info overlay) and the company's **About tab specifically** (never the Home tab — forced by `toCompanyAboutUrl`, since About is where company facts actually render). Pass `--hops ""` to `tools/linkedin-enrich-scrape.js` for a single-page-only run if account risk needs to be dialed back |
| Max page loads per URL attempt | 4 (source + profile main + profile Contact info overlay + company About tab) — enforced inside `tools/linkedin-enrich-scrape.js`, not agent reasoning |
...
| PATCH response log | `./logs/crm-leads-enrich-responses.md` (gitignored) |
